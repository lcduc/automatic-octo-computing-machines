...
def _result(source_id: str, score: float, source_name: str = None, chunk_id: str = "chunk_001") -> dict:
    return {
        "index": 0,
...
def test_citations_dedupes_by_source_keeping_best_score():
    results = [
        _result("doc_a", 0.6, chunk_id="chunk_001"),
        _result("doc_a", 0.9, chunk_id="chunk_004"),  # better chunk from the same source
        _result("doc_b", 0.75, chunk_id="chunk_002"),
    ]

    citations = ChatResponseFactory.citations(results)

    assert citations == [
        {"source": "doc_a", "type": "file", "score": 0.9, "chunk_id": "chunk_004"},
        {"source": "doc_b", "type": "file", "score": 0.75, "chunk_id": "chunk_002"},
    ]
...
def test_history_payload_carries_answer_and_citations():
    ...
    assert payload["citations"] == [
        {"source": "doc_a", "type": "file", "score": 0.9, "chunk_id": "chunk_001"}
    ]
