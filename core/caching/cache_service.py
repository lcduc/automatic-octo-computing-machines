...
class SmartCacheService:
    ...
    def __init__(self, max_size: int = None, ttl_seconds: int = None):
        ...
        self._cache_file = os.path.join(FileConfig.TEMP_DIR(), "smart_cache.pkl")
    ...
    def _evict_lru(self):
        ...
        logger.debug(f"🗑️ Evicted {entries_to_remove} LRU cache entries")
    ...
    def _find_similar_query(self, query: str, similarity_threshold: float = 0.9) -> Optional[str]:
        ...
        try:
            ...
            logger.debug(f"⚠️ Similarity search failed: {e}")
    ...
    def set(self, query: str, response: Any, context_hash: str = "", 
        ...
        try:
            ...
            logger.debug(f"⚠️ Failed to generate query embedding: {e}")
