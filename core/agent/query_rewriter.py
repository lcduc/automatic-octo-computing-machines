...
class QueryRewriter:
    ...
    def rewrite(self, query: str, history: Optional[List[Dict[str, str]]]) -> str:
        ...
        if rewritten != query:
            # Truncated like every other query log in the codebase: user queries
            # can carry sensitive phrasing and file logging is on by default.
            logger.debug("Query rewritten: %r -> %r", query[:50], rewritten[:50])
        return rewritten
