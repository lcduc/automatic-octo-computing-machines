...
class ValidationUtils:
    ...
    def validate_file_extension(
        ...
        allowed_extensions = allowed_extensions or FileConfig.ALLOWED_EXTENSIONS()
    ...
    def validate_batch_size(file_count: int, total_size: int) -> Dict[str, Any]:
        ...
        # Check file count
        if file_count > FileConfig.MAX_FILES_PER_BATCH():
            return {
                "valid": False,
                "error": f"Too many files. Maximum {FileConfig.MAX_FILES_PER_BATCH()} files per batch.",
            }
        ...
        if total_size > FileConfig.MAX_TOTAL_BATCH_SIZE():
            return {
                "valid": False,
                "error": f"Total batch size too large. Maximum {FileConfig.MAX_TOTAL_BATCH_SIZE() / (1024*1024):.1f}MB allowed.",
            }
