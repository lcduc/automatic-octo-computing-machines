...
def cleanup_data_folders():
    ...
    for dir_path in cleanup_dirs:
        try:
            if FileUtils.directory_exists(dir_path):
                # Use FileUtils for consistent cleanup operations
...
def cleanup_logs():
    ...
    try:
        if FileUtils.directory_exists(LoggingConfig.LOG_DIR()):
            ...
            logger.warning(f"⚠️ Logs directory not found: {LoggingConfig.LOG_DIR()}")
            return 0
    except Exception as e:
        ...
        for dir_path in sorted(
            ...
            if dir_path.is_dir() and dir_path != path:
                try:
                    ...
                    logger.warning(
                        f"⚠️ Could not remove empty directory {dir_path}: {e}"
                    )
